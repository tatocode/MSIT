...
def save_test_prediction(image: np.array, prediction: np.array, cmap: np.array, save_path: str) -> None:
    assert image.shape[:-1] == prediction.shape, f'测试集 image({image.shape[:-1]}), ' \
                                                               f'predict({prediction.shape}) 形状大小不一致'
    result = cv2.addWeighted(image, 0.5, cmap[prediction], 0.5, 0, dtype=cv2.CV_32F).astype(np.uint8)
    cv2.imwrite(save_path, result)
