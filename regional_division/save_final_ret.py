...
def main(model_name, model, image_path, mask_path, label_path):
    ...
    out = cv2.resize(out, (w, h), interpolation=cv2.INTER_NEAREST).astype(np.uint8)
    ...
    # out = np.stack([out, out, out], axis=2)*255

    r = cv2.addWeighted(img, 0.5, ret.astype(np.uint8), 0.5, 0)
    global new_index
    cv2.imwrite(f'temp/ret_{new_index}.png', r)
    new_index += 1
...
if __name__ == '__main__':
    ...
    for mn in model_names:
        ...
        root_dir = os.path.join(temp_dir, '230904_163950')
        ...
        for fold in range(4):
            ...
            dataset_file = os.path.join('/media/dell/DATA/dataset_20230924/dataset_20230630/regional_division', f'fold_{fold}.txt')
            ...
            for _c in c:
                img_path = f'/media/dell/DATA/dataset_20230924/dataset_20230630/regional_division/images/{_c.strip()}.png'
                label_path = f'/media/dell/DATA/dataset_20230924/dataset_20230630/regional_division/annos/{_c.strip()}.json'
                mask_path = f'/media/dell/DATA/dataset_20230924/dataset_20230630/regional_division/masks/{_c.strip()}.png'
